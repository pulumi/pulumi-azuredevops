...
class ServiceendpointJfrogXrayV2(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if authentication_basic is not None and not isinstance(authentication_basic, ServiceendpointJfrogXrayV2AuthenticationBasicArgs):
                authentication_basic = authentication_basic or {}
                def _setter(key, value):
                    authentication_basic[key] = value
                ServiceendpointJfrogXrayV2AuthenticationBasicArgs._configure(_setter, **authentication_basic)
            __props__.__dict__["authentication_basic"] = authentication_basic
            if authentication_token is not None and not isinstance(authentication_token, ServiceendpointJfrogXrayV2AuthenticationTokenArgs):
                authentication_token = authentication_token or {}
                def _setter(key, value):
                    authentication_token[key] = value
                ServiceendpointJfrogXrayV2AuthenticationTokenArgs._configure(_setter, **authentication_token)
