...
class Kubernetes(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if kubeconfig is not None and not isinstance(kubeconfig, KubernetesKubeconfigArgs):
                kubeconfig = kubeconfig or {}
                def _setter(key, value):
                    kubeconfig[key] = value
                KubernetesKubeconfigArgs._configure(_setter, **kubeconfig)
            __props__.__dict__["kubeconfig"] = kubeconfig
            if project_id is None and not opts.urn:
                raise TypeError("Missing required property 'project_id'")
            __props__.__dict__["project_id"] = project_id
            if service_account is not None and not isinstance(service_account, KubernetesServiceAccountArgs):
                service_account = service_account or {}
                def _setter(key, value):
                    service_account[key] = value
                KubernetesServiceAccountArgs._configure(_setter, **service_account)
